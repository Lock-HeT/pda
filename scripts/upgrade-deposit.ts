...
async function main() {
  ...
  // 获取新的合约工厂
  const PDADepositUpgradeableV2 = await ethers.getContractFactory('PDADepositUpgradeable');

  // 验证升级兼容性
  console.log('Validating upgrade...');
  try {
    await upgrades.validateUpgrade(PROXY_ADDRESS, PDADepositUpgradeableV2);
  ...
  const upgraded = await upgrades.upgradeProxy(PROXY_ADDRESS, PDADepositUpgradeableV2);
  ...
  const deposit = await ethers.getContractAt('PDADepositUpgradeable', PROXY_ADDRESS);
  
  try {
    const version = await deposit.version();
