...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  console.log('\n========================================');
  console.log('Deploying PDALiquidityManager Contract (UUPS)');
  console.log('========================================\n');
  ...
  try {
    ...
    try {
      const existingDeployment = await deployments.get('PDALiquidityManager');
      proxyAddress = existingDeployment.address;
      
      console.log('   Existing proxy found, importing...');
      // 使用 forceImport 导入已存在的代理
      await upgrades.forceImport(proxyAddress, PDALiquidityManager, { kind: 'uups' });
      liquidityManager = await ethers.getContractAt('PDALiquidityManager', proxyAddress);
    ...
    const tx = liquidityManager.deploymentTransaction();
    if (tx) {
    ...
    proxyAddress = await liquidityManager.getAddress();
    const implementation = await upgrades.erc1967.getImplementationAddress(proxyAddress);

    console.log('✅ PDALiquidityManager deployment completed!');
    console.log(`   Proxy Address: ${proxyAddress}`);
    console.log(`   Implementation Address: ${implementation}`);

    // 保存为 hardhat-deploy deployment 文件
    const artifact = await deployments.getArtifact('PDALiquidityManager');

    await deployments.save('PDALiquidityManager', {
      abi: artifact.abi,
